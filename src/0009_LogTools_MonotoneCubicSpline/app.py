...
def upload_file():
   if request.method == 'POST':
      ...
      if len(f.filename.strip()) > 0:
         ...
         rawData = {"lable":[], "chartData":[]}
         if f.filename == "data.txt":
            with open("workspace/data.txt") as file:
               for line in file:
                  rawData["lable"].append(float(line.split(",")[0]))
                  rawData["chartData"].append(float(line.split(",")[1]) * 255)

            cs = CubicSpline(rawData["lable"], rawData["chartData"])

            xs = np.arange(0, rawData["lable"][-1], 1)
            rawData["lable"] = xs.tolist()
            rawData["chartData"] = cs(xs).tolist()
